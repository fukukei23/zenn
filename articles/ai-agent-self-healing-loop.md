...
graph TD
    A[テスト実行] --> B{結果}
    B -->|成功| C[次フェーズへ]
    B -->|失敗| D["DebuggerAgent<br/>エラー分析 + 修正パッチ生成"]
    D --> G2["GuardianAgent<br/>パッチレビュー"]
    G2 --> E[パッチ適用 → 再テスト]
    E --> F{結果}
    F -->|成功| G["PostmortemAgent<br/>知識ベース更新"]
    F -->|失敗・リトライ上限| H[人間に通知]
```
...
:::message
Auto-PRは便利ですが、レビューなしの自動マージは本番環境に悪影響を及ぼすリスクがあります。必ず人間の承認フローを挟むことを推奨します。
:::
...
:::message alert
APIキーやシークレットの漏洩は、LLMの学習データに組み込まれるリスクがあります。redact処理は必ずLLM呼び出しの前段で実行し、パッチの差分にも機密情報が含まれていないかGuardianAgentが検査します。
:::
...
```mermaid
graph TD
    A[エラー発生] --> B["DebuggerAgent: 既知の解決策を検索"]
    B -->|なし| C["LLMで分析・修正生成"]
    C --> D["パッチ適用 → 解決"]
    D --> E["PostmortemAgent: シグネチャ + 解決パターン生成"]
    E --> F["知識ベースに保存（JSON）"]
    F --> G["次回 同種エラー発生"]
    G --> H["DebuggerAgent: 知識ベースから即座に解決"]
    style H fill:#90EE90
```
...
- **誤修正のリスク**: LLMが生成するパッチが必ずしも正しいとは限らない。GuardianAgentが差分の妥当性（構文エラー・意図しない削除・機密情報の混入）を検査するが、論理的な誤りまでは検出できない場合がある
