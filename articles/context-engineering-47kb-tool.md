...
GPT-4のコンテキストウィンドウは128Kトークン（約500KB）。Claudeなら200Kトークン（約800KB）。
47KBは**LLMのコンテキストに丸ごと載せて、なお余裕がある**サイズです。

つまり、contextforge自身のコードをLLMに見せれば——

> 「このコードを読んで、バグを見つけて」
> 「このコードを読んで、テストを書いて」
> 「このコードを読んで、リファクタリングして」

——**コード自身が自分自身の説明書になる**。これが47KBの真意です。

依存ライブラリがゼロなので、外部ファイルを別途読み込む必要もありません。
1つの `.py` ファイル = LLMにとって完結した1つの世界。
...
これらはTDDの「レッド→グリーン→リファクタリング」と同じように、Context Engineeringにも**再現可能な手順**があります。以下の5つを意識するだけで、LLMの回答精度が体感で変わります。
...
| Claude Code | CLI・プロジェクト全体分析 | npm必要 | × |
