---
title: "AIの答えを、そのまま「正解」にしない開発ループ"
emoji: "🔄"
type: "tech"
...
- 実装が終わっても、「本当に大丈夫か？」を確かめる**決まった手順がなかった**。確認するかどうかが、その日の根気と注意力次第になっていた

この記事では、これを解決するために組み立てた**「AIの答えをそのまま正解にしない開発ループ」**を紹介します。核心はシンプルで、次の1文に尽きます。

> **AIの判断を「そのまま採用」せず、「複数AIに叩かせて、実測で裁定する」工程をループに組み込む**

## 全体フロー

開発は**2層のループ**で回しています。外側が「毎日の運用」、内側が「1つのタスクをやる時の手順」です。
...
朝には昨日夜の自動実行結果の報告が届き、それを見てから今日やることを選びます。「夜の間に準備が整っていて、朝は選ぶだけ」が1日の始まりです。
...
「何を始めるか」はタスクの入口（自動化作成・新機能・是正・記事執筆・意思決定…）で切り、入口ごとに使うスキルの組み合わせを一覧表に決めておくと、毎回の手順検討が不要になります。
...
## 実例：2ラウンドで87指摘→採用11件→「もう出ない」まで減らす
...
ここで「**収束**」という言葉を定義しておきます。AIレビューの文脈では、**「同じ指摘が出なくなるまで減ること」**を指します。無限にレビューを回すことではありません。

面白いのは指摘の**減り方**です。r1で直した7件の**同じ指摘の再発はゼロ**（r2が指摘したのは修正の「周辺の穴」——stage1だけ直してinbox側が残っていた等）で、r2の新規highは2件まで減り、残るはlow/nitと設計判断領域へ移行しました。これが「**収束**」です。AIレビューは「回せば回るほど正しくなる」のではなく、**実装者=検証者の偏りを複数AIとblind再レビューで構造的に削る**ことで初めて機能します。
...
- AI開発の生産性は「AIに何を頼むか」より「**AIの意見をどうひとつに決めるか**」で決まる
- 核心: **壁打ち→点数評価→複数AIレビュー→TDD→blind再レビュー→実測で決める→記録**のループ（外側に毎日の運用ループを回す）
- レビューは回数でなく**減り方**を見る（同じ指摘の再発ゼロ・重要な新規指摘の減少）
