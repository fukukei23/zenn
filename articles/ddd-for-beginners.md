...
**開発者とビジネス側で同じ言葉を使う** принцип。

「予約」と「予定」、这两种在业务上可能是不同的概念，但如果没有明确的区分，代码中就会混乱。DDDではコード内の名前も業務用語に合わせます。
...
| ✅ ルールが頻繁に変わる（業務知識の変化に追従したい） | ❌ データの変換・加工 중심 |
...
DDD的核心は**「ビジネスルールを正確にコードで表現する」**こと。
