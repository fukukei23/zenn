...
graph LR
    ...
    Classifier --> Profile1["gpt_codex<br/>GPT-5.5"]
    Classifier --> Profile2["sonnet_review<br/>Sonnet 4.6"]
    Classifier --> Profile3["glm_default<br/>GLM-5.1"]
    Classifier --> Profile4["minimax_chat<br/>MiniMax M2.7"]
...
graph TD
    ...
    T1 --> Cov{"Coverage 80%+"}
    T1 --> Pylint{"Pylint 8.0+"}
    T1 --> Mypy{"Mypy Pass?"}
    T1 --> Bandit{"Bandit Pass?"}

    Cov -->|Pass| T1OK["Tier 1 通過"]
    Pylint -->|Pass| T1OK
    Mypy -->|Pass| T1OK
    Bandit -->|Pass| T1OK
    ...
    T1OK --> T2["Tier 2: Mutation Test"]
    T2 --> MutScore{"Mutation Score 70%+"}
...
graph LR
    ...
    Council --> Validate["amendment検証"]
    Validate --> Policy["Policy Store更新"]
    Validate --> Archive["アーカイブ"]
